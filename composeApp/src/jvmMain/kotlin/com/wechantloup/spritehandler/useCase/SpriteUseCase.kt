...
object SpriteUseCase {
        ...
        val pixels: List<Int> = images.flatMap { image ->
            ...
            List(width * height) { index ->
                ...
                if (imgX < 0 || imgX >= image.width || imgY < 0 || imgY >= image.height) {
                    ...
                    palette.colors.indexOf(color.value)
                }
            }
        }
    ...
    private fun generatePalette(images: List<Image>): Palette {
        ...
        val colors = mutableListOf(Color.Transparent.value)
...
//                    }
                    if (!colors.contains(ulongColor)) {
//                        logger.info("new color found $color")
                        colors.add(ulongColor)
                    }
                }
            }
        }
        ...
        if (colors.size < 16) {
            (colors.size until 15).forEach { _ ->
                colors.add(Color.Unspecified.value)
            }
        }
    ...
    private fun checkPalette(images: List<Image>, colorList: List<ULong>): Palette {
    ...
    private fun ULong.toBytes(): List<Byte> = (7 downTo 0).map { i ->
        ((this shr (i * 8)) and 0xFFu).toByte()
    }

    private fun List<Byte>.toULong(): ULong = fold(0uL) { acc, byte ->
